...
## 11. P11+ 更新 (Week 10 之后)

- **P11-1.5**: `convert_input` 派生现在发完整事件链 (RunStart + UserInput + ModelResponse); 修 2 unit test fail + 1 smoke test; dsh_synthetic 28.6% → 100%.
- **P11-2**: dsh 9 acp-snapshot fixture (真 dsh 仓库, 不是合成) → 9/9 = 100% via Python 转换脚本 (`dsh_snap_convert.py`). `mah conformance` 加 `--dsh` flag.
- **P12-1**: `DshFixtureCache` — dsh_snap.jsonl 的 mtime 失效机制; 4/4 cache tests.
- **P12-3**: `docs/README.md` 成为主索引; `mkdocs.yml` v2 准备.
- **P12-9**: `mah conformance` exit code: pass rate < 95% → exit 1 (CI gating).

见 `docs/p11-final-report.md`, `docs/p12-final-report.md`, `docs/dsh-benchmark-report.md`.
